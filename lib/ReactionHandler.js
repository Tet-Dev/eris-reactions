...
class ReactionHandler extends EventEmitter {
    ...
    checkPreConditions(msg, emoji, reactor) {
        ...
        if (this.filter(reactor.id || reactor)) {
            console.log(emoji,reactor)
            this.collected.push({ msg, emoji, userID: reactor, reactor: reactor.id ? reactor : { id: reactor } });
            this.emit('reacted', { msg, emoji, userID: reactor, reactor: reactor.id ? reactor : { id: reactor } });
